...
export const collection = (paths: string[]) =>
    fCollection(firestore, paths[0] as string, ...paths.slice(1));
export const doc = (paths: string[]) =>
    fDoc(firestore, paths.shift() as string, ...paths);

export const getDoc = async (paths: string[]) => await fGetDoc(doc(paths));
export const addDoc = async (paths: string[], data: any) =>
    await fAddDoc(collection(paths), data);
...
export const queryPagination = async ({
    ...
    if (!queryRef)
        throw new Error('[queryPagination] - are not assigned queryRef');
